...
def preprocess_mnist_image(image, label):
    # Resize 28x28 to 128x128, keep single channel (grayscale)
...
# ==========================================
def get_mini_resnet_base(input_shape=(128, 128, 1)):
...
has_pretrained = (
    os.path.exists("models/mini_resnet_pretrained_weights_128.h5") or 
    os.path.exists("models/mini_resnet_pretrained_weights_96.h5") or 
    os.path.exists("models/mini_resnet_pretrained_weights.h5")
)
...
src_weights_path_128 = "models/mini_resnet_pretrained_weights_128.h5"
src_weights_path_96 = "models/mini_resnet_pretrained_weights_96.h5"
src_weights_path_default = "models/mini_resnet_pretrained_weights.h5"

if os.path.exists(src_weights_path_128):
    print(f"Loading pre-trained 128x128 weights from {src_weights_path_128}...")
    temp_base_128 = get_mini_resnet_base(input_shape=(128, 128, 1))
    temp_inputs_128 = tf.keras.Input(shape=(128, 128, 1))
    temp_x_128 = temp_base_128(temp_inputs_128, training=True)
    temp_x_128 = tf.keras.layers.Dropout(0.5)(temp_x_128)
    temp_outputs_128 = tf.keras.layers.Dense(24, activation='softmax')(temp_x_128)
    temp_model_128 = tf.keras.Model(temp_inputs_128, temp_outputs_128)
    temp_model_128.load_weights(src_weights_path_128)
    
    # Copy weights
    for layer in ft_resnet_base.layers:
        if layer.name.startswith('conv') or layer.name.startswith('shortcut_conv'):
            src_layer = temp_base_128.get_layer(layer.name)
            layer.set_weights(src_layer.get_weights())
    print("Successfully loaded and copied 128x128 pre-trained weights.")
else:
    src_weights_path = src_weights_path_96 if os.path.exists(src_weights_path_96) else src_weights_path_default
    print(f"Loading pre-trained 96x96 weights from {src_weights_path}...")
    ...
    for layer in ft_resnet_base.layers:
        if layer.name.startswith('conv') or layer.name.startswith('shortcut_conv'):
            ...
            layer.set_weights(src_layer.get_weights())
    print("Successfully copied pre-trained 96x96 convolutional weights into the 128x128 base model.")
...
ft_model_path = "models/Mini_ResNet_finetuned_128.keras"
ft_onnx_path = "models/Mini_ResNet_finetuned_128.onnx"
