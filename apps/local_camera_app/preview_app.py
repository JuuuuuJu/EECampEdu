...
inference ``RESULT`` line, and forwarding the chosen robot action to the ESP2
...
DEFAULT_BAUD = 115200
# Robot-arm actions, index-aligned with the ESP2 output firmware apply_action() table.
OUTPUT_ACTIONS = ["up", "down", "left", "right", "clamp", "release"]

# Shared latest-inference state, updated by the ESP1 reader thread.
...
_serial_handles = {"esp1": None, "esp2": None}
_reader_thread = None
...
def _reader_loop(esp1):
    mapping = _index_to_action()
    while True:
        try:
            raw = esp1.readline().decode("utf-8", "replace").strip()
        ...
        if action and _serial_handles["esp2"] is not None:
            try:
                _serial_handles["esp2"].write((action + "\n").encode("utf-8"))
...
def create_app():
    ...
    def connect():
        ...
        data = request.get_json(silent=True) or {}
        esp1_port = data.get("esp1_port")
        esp2_port = data.get("esp2_port")
        baud = int(data.get("baud", DEFAULT_BAUD))
        if not esp1_port:
            return jsonify({"ok": False, "error": "esp1_port required"}), 400
        global _reader_thread
        try:
            esp1 = serial.Serial(esp1_port, baud, timeout=1)
            _serial_handles["esp1"] = esp1
            if esp2_port:
                _serial_handles["esp2"] = serial.Serial(esp2_port, baud, timeout=1)
        except Exception as exc:  # noqa: BLE001 - open failure
            return jsonify({"ok": False, "error": f"open failed: {exc}"}), 502
        with _state_lock:
            _state.update(connected=True, esp1_port=esp1_port, esp2_port=esp2_port, error=None)
        _reader_thread = threading.Thread(target=_reader_loop, args=(esp1,), daemon=True)
        _reader_thread.start()
        return jsonify({"ok": True, "esp1_port": esp1_port, "esp2_port": esp2_port})
...
def main():
    ...
    print("[preview_app] POST /connect with {esp1_port, esp2_port?} after listing /ports.")
